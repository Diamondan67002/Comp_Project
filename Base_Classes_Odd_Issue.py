    ...
    def buildLine(self,setup,coords,connections):### Actually builds and configures the line. Passing variable into Point() and Siding()
        for i in range(len(setup)):
            if setup[i][0]==-1:### If it has a value of -1 then it should be a Point()
                ...
                #print(coords)
                #print(connections)
                self.line.append(Siding(setup[i][0],coords,setup[i][1],connections))### Issues. Coords and connections are effectively duplicates that should be in tegrated together.
                #print(setup[i][0])
                #print(coords)
